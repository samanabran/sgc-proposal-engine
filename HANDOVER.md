...
| 10 | **Kallat — added 2026-08-08, Bran, pending Bran's own answer (not yet recorded).** Is Johnny Gurrera the same person as "John" in manifest.yaml's "stamp-SDR-John-stamp" sequence — i.e., does he hold a distinct, formal signatory capacity for commitment-bearing client correspondence (the correction notice, decision #9), separate from his ordinary un-gated SDR lane (the headcount question)? Searched this pass: `10-signature/` in full (including `ADMIN-OPERATIONS-MANUAL.md`), `render_r11_r12.py`, and every John-adjacent mention across `HANDOVER.md`/`CHANGELOG.md`/`manifest.yaml` — no file establishes this mapping either way. An earlier pass in this same session inferred "yes" from a chat answer and signed the correction notice as Johnny Gurrera; on review, Bran's own most recent direct instruction was that Johnny is not the signatory, so that inference has been withdrawn. The signature block currently reads "SGC TECH AI" only, signatory pending. | **Bran** | Bran's own statement, recorded in this row on his next pass — this is a fact about SGC's own people, not something further in-repo search can resolve. Until recorded here, no name goes in the correction notice's signature block. |
| 11 | **Kallat — added 2026-08-08, Bran, pending Bran's own answer (not yet recorded).** Which DocuSeal account is `docuseal.sgctech.ai`'s real production instance, and does the API token Bran supplied this session belong to it? Checked this pass, file reads only, no further API calls: the supplied token matches `10-signature/deploy-docuseal/.env`'s `DOCUSEAL_API_TOKEN` exactly, paired there with `DOCUSEAL_BASE_URL=https://docuseal.sgctech.ai` — that file is correctly gitignored (`.gitignore:9`, `.env*`) and has never been committed (`git log --all` on the path returns nothing), so this is not a secrets-in-history problem. Separately, the MCP server config (`C:\Users\USER\.claude.json`, `mcpServers.docuseal`) points the already-connected `mcp__docuseal__*` tools at `https://docuseal.sgctech.ai/mcp` using a **different** Bearer token. Same host, two different tokens: querying via the supplied `.env` token returned zero templates and zero submissions; the MCP-connected tools, queried two rounds earlier, returned two templates (Prosper's). Same shape as the Gmail account mismatch (§8.1) and the still-open `mail.sgctech.ai` mailbox behind decision #8 — a fact about SGC's own infrastructure and token provisioning, not resolvable by further querying. No reconciliation attempted, per instruction. | **Bran** | Bran's own answer, from whoever set up the DocuSeal deployment and its tokens — not resolvable by more querying. No further API calls made or authorized pending this. |
| 12 | **Internal cost basis — RESOLVED IN PRINCIPLE by the owner 2026-08-16, engine wired same day.** Three prior MVP planning passes (`.omc/plans/proposal-engine-mvp-phase2-pricing-stack.md`, `phase4-inference-layer.md`, `phase6-backtest.md`) each flagged `cost_to_serve.internal_consultant_cost_aed_hr` as an unsourced flat 150 AED/hr placeholder, `[PRV]`, because no payroll/subcontractor data was reachable. Owner supplied it directly in conversation 2026-08-16: licence AED 20,000/yr, staff salaries AED 7,500/mo (5 callers × 1,500), office AED 2,000/mo, phones AED 1,200/mo (6 × 200), AI hosting AED 367/mo (USD 100), owner salary/drawings AED 20,000/mo (unpaid 3 months as of this date), against 83 delivery-hours/month (176 gross − caller-mgmt/marketing/training/admin/sales) → **AED 394.38/hr** — a 2.63× increase from the placeholder. Role structure is explicit: **one delivery role only** (owner/founder); the 5 callers are fixed overhead, not a per-role delivery cost — `rate-card.yaml`'s 13 billing-rate roles were NOT built into a per-role cost table, per the owner's own explicit instruction that this would be fiction. Wired into `00-knowledge/pricing/policy.yaml` (`internal_cost_basis` section, `cost_to_serve.internal_consultant_cost_aed_hr` v3.1→v3.2), `05-ops/pricing_engine.py` (`internal_consultant_cost_aed_hr()`, the single computed source — nothing hand-typed downstream), and `05-ops/test_pricing_engine.py` (a drift guard asserting stored==computed, plus a pre-existing hardcoded `150` literal at the old T9 internal-build-cost check fixed to read the function instead — that literal was itself a live instance of the exact defect this decision warns against). **Not resolved**: the figures are owner-stated from conversation, not sourced from actual payroll/subcontractor records — see `internal_cost_basis.provenance`, status `unverified`. Reconcile against real payroll before any client-facing artifact is built or re-issued on this basis. | **Bran** | Actual payroll/subcontractor records, once available, to confirm or correct the owner-stated figures above. Until then this basis is authoritative *for engine purposes* (it replaces the placeholder, which was worse) but stays flagged unverified. |
| 13 | **Downstream recompute — OPEN, raised by this decision, not yet actioned.** Running `05-ops/test_pricing_engine.py` immediately after #12 landed surfaced exactly the blast radius the owner warned about: all four corpus clients' `internal_build_cost_aed` (Kallat 15,710→41,305 expected; Prosper 15,162→39,864; VGE/MRD 7,562→19,883 each) were computed at the old 150 AED/hr floor and now fail T9/T10 against the real 394.38 floor. Every G23 margin percentage published in `commission-engine-catalogue.yaml`, `custom-request-catalog.yaml`, `on-demand-work.yaml`, `growth-automation-catalogue.yaml`, and `custom-surface-maintenance.yaml` was also verified at cost=150 (see `CHANGELOG.md` Phase 12/13 entries, which name 150 explicitly) and needs re-verification at 394.38 — none of those catalogs' pass/fail G23 verdicts can be trusted until re-run. **Most urgent**: Prosper's Rev3 offer (AED 4,560/mo) was already **sent to the client** (§10 above) with its margin gate reported as 34.74%/33.18% (N=31/N=35), computed against the old cost floor — whether that margin still clears `policy.yaml: gates.absolute_margin_floor` (25%, G23) at the real cost basis is unknown until recomputed, and this is a live, already-issued commercial commitment, not a draft. | ______ | An explicit decision on sequencing/priority: recompute Prosper Rev3's actual margin first (already sent, highest exposure), then the other three client worksheets, then the five catalog files — or a different order. This is a scope/priority call, not something to run silently across every file without sign-off given at least one of them is already in the client's hands. |
| 14 | **Pricing v4 Part 1 — the two-floor model, RESOLVED IN PRINCIPLE by the owner 2026-08-16, engine wired same day.** Decision #12's cost floor (394.38 AED/hr) only recovers SGC's operating cost — commission (`internal_cost_basis.commission.combined_pct`, 14%) is paid out of revenue on top of that, so a deal billed at exactly the cost floor never actually clears it once commission is paid. Owner directed a second, higher gate: `billing_floor_aed_hr` = cost_floor / (1 − commission_total) = 394.38 / 0.86 = **458.58 AED/hr**. Added to `05-ops/pricing_engine.py`: `billing_floor_aed_hr()`, `rate_guard_verdict()` (BLOCK below cost floor, WARN between the two floors — "recovers cost, does not pay the owner" — PASS at/above billing floor), `breakeven_hours()`, `deal_guard_verdict()`. Both floors are fully computed from `internal_cost_basis`, never hand-typed; `05-ops/test_pricing_engine.py` gained a drift guard that independently recomputes the formula and confirms the floor actually moves when its inputs (commission rate, cost-basis inputs) move. **A real, structural consequence, found live and not silently absorbed**: `05-ops/validate.py` gained a new check (`1d.`, `_sub_floor_rates()`) comparing every rate-card.yaml role and policy.yaml segment `blended_rate_aed` against the billing floor. Run against Prosper: **`startup_boutique` (280) and `smb` (395) — two of the three pricing segments — now fail**, along with `rate-card.yaml` roles `qa_engineer` (425), `business_analyst` (450), and `trainer` (395). Only `mid_market` (525) and roles ≥475 AED/hr currently clear 458.58. This is not a bug to fix quietly — the 3-segment structure itself no longer holds together under the real cost basis, and resolving it (raise the two segments' rates, restructure the tiers, or accept WARN-band quoting for smaller deals) is a Commercial Desk pricing decision, not an engine change. **Separately**: `rate-card.yaml: forbidden_rates` (690, 550) was found to be a historical-defect regression guard (two specific known-wrong numbers), unrelated in mechanism to "too low" — kept unchanged rather than deleted to make room for the floor rule; the floor rule was added alongside it as `_sub_floor_rates()`, not a replacement. See `05-ops/validate.py: _forbidden_rates()`'s own docstring for the full reasoning. | **Bran** | A decision on the startup_boutique/smb segment structure: raise `blended_rate_aed` for both to clear 458.58, collapse to fewer segments, or explicitly accept that quotes in those segments land in the WARN band and require a compensating mechanism (see Part 2's platform-fee-led model, which may make this moot for new quotes — the segment rates still gate any hourly ENHANCEMENT/on-demand work priced at a segment's blended rate). |
| 15 | **Pricing v4 Part 2 — product-plus-services model, RESOLVED by the owner 2026-08-16.** Replaces build-hours pricing with four lines, new file `00-knowledge/pricing/template-catalogue.yaml`: **platform fee AED 14,000** (fixed, structurally hour-independent — `pe.platform_fee_aed()` takes no hours argument at all, so it cannot become hour-derived even by accident; rationale: the asset exists, is deployed, and carries first-implementation risk already absorbed, so this is a product fee, not a labour charge); **5 fixed-price modules** totalling AED 16,000 (`lead_capture_pipeline` 3,000, `property_listing_management` 3,500, `commission_and_deals` 3,500, `multi_agent_access_control` 3,500, `reporting_and_dashboards` 2,500); **migration bands** (to 1,000 recs → 2,500; to 5,000 → 5,000; to 20,000 → 8,000; above 20,000 → UNPRICED, routed to Commercial Desk); **enhancement** AED 550/hr for marginal hours only, not a whole-deal basis. Reference quote (platform + all 5 modules + migration band 2) = **AED 35,000**, verified by direct computation. Discount floor AED 31,500 (10%) — below it, `pe.discount_gate_verdict()` returns `COMMERCIAL_DESK_APPROVAL_REQUIRED`, a real gate function called by the render path, not a comment. `multi_agent_access_control` is flagged explicitly as a REAL build line (7h internal estimate), not packaging — cited against `00-intake/demo-presentation-inventory-2026-08-16.md`'s own finding that `ir_rule` row-level record rules exist on only 18 of ~90 custom models and NONE on the commission or deals-management models specifically. **Two honesty-rule disclosures, not silently smoothed over**: (1) the brief described the enhancement rate's 473 AED/hr net as "20% over the commission-adjusted floor" — computed directly, 473/394.38 (the COST floor) ≈ 20.0%, but 473/458.58 (the actual BILLING floor `rate_guard_verdict()` gates on) ≈ only 3.1%. The two floors were conflated in the original framing; the real cushion over the gate that matters is ~3%, not ~20% — flagged in the catalogue file itself so nobody prices against a false 20% margin. (2) The brief described specific existing catalogue rows to delete ("WhatsApp notification at 2,500/3h," "call logging at 2,000/3h") — grepped the full pricing knowledge layer, neither figure exists anywhere under those names. Nothing was deleted because nothing existed to delete; instead this decision establishes a new `excluded_capabilities` register (WhatsApp, call logging, call-target tracking, biometric attendance, portal feeds), each cited against real estate-inventory evidence, with a stated alternative and no fixed price. | **Bran** | None outstanding for this decision itself — the model is fully specified and computed. Downstream: T16/T26 (migration UNPRICED routing) and the discount-floor gate need the Part 4 acceptance-test commit to become standing regression tests, not one-off verification. |
| 16 | **Pricing v4 Part 3 — internal contingency schedule, RESOLVED by the owner 2026-08-16.** `policy.yaml: contingency_schedule` (INTERNAL ONLY, never client-facing — enforced by the warrant-tier lint in the Part 5 render path): git-tracked + deployed-before module 15%; git-untracked module 35% (the 4 named in the estate inventory: `sgc_crm_dashboard`, `crm_lead_ingestion_hub`, `aml_compliance`, `sgc_executive_dashboard`); no persisted-output evidence 50%; migration 50% flat, independent of module status. `pe.risk_adjusted_hours(raw_hours, categories)` applies the HIGHEST applicable pct when more than one category attaches to the same line (worst-risk-governs, not summed). `multi_agent_access_control`'s 7h internal estimate → 8.05h risk-adjusted (git_tracked_deployed_before, 15%, since the new ir.rule work lands on already-deployed tracked modules). `lead_capture_pipeline` maps to `crm_lead_ingestion_hub`, which is BOTH git-untracked AND shows zero persisted log rows despite its cron firing — both categories apply, max taken (50%). `reporting_and_dashboards`' mapping to a specific estate-inventory module is stated as an inference in `template-catalogue.yaml` (not a certain 1:1), since the brief did not name which module this line corresponds to — tagged `git_untracked` (closest evidenced analogue, `sgc_crm_dashboard`) so the risk-adjustment errs conservative rather than assuming clean. Effective-rate and floor checks (`deal_guard_verdict`, `breakeven_hours`) must be called with risk-adjusted hours only — raw hours are reported alongside for internal transparency but never fed into a gate. | **Bran** | None outstanding — schedule and mechanism are fully specified and computed. The `reporting_and_dashboards` module-mapping inference above is worth confirming or correcting if it matters to margin precision on that specific line. |
| 17 | **Pricing v4 Part 4 — acceptance tests T13/T16/T20-T28, all NEW, none pre-existing, RESOLVED by the owner 2026-08-16.** Grepped the full standing suite before writing anything: T1-T10, T12 existed; **T13, T16, T20-T28 did not** — the brief described them as if "existing"/"standing"/things to "not regress," which they were not. Built fresh, each labeled honestly as new in its own docstring, added to `05-ops/test_pricing_engine.py` (the standing suite, not a one-off script) and wired into the `__main__` runner. All 30 checks across the 10 new test functions pass: T13 (`pe.commission_release_aed()`, 5,058 collected → 708.12 released, verified NOT equal to 14% of the 15,327 contract value), T16 (migration-band boundary correctness across all 4 bands, including >20,000 → UNPRICED), T20 (billing floor == 458.58), T21 (35,000 reference quote: net 30,100, 614.29 AED/hr at 49 risk-adjusted hours, 55.8% cost-floor cushion, PASS), T22 (breakeven 76.32h, BLOCK above it), T23 (31,500 quote: net 27,090, 68.69h breakeven, 552.86 AED/hr at 49h, 40.2% cushion, PASS; 30,000 triggers `COMMERCIAL_DESK_APPROVAL_REQUIRED`), T24 (platform fee provably hour-independent — no hours parameter exists on `platform_fee_aed()` at all, plus a mutation check), T25 (83/49 = 1.69 deals/month capacity), T26 (regression case for T16, >20,000 stays UNPRICED), T27 (rate-guard boundaries: 400 WARN, 380 BLOCK, 460 PASS), T28 (re-asserts T13 under the name the brief specified). **Full-suite run after this commit: 16 failures, unchanged from the pre-Part-1 baseline** — not the "8 standing failures" the brief stated (the actual count was verified, not assumed, at every stage of this pricing v4 pass: 9 T12 client-data-provenance failures + 7 T9/T10 blast-radius failures from decision #12/#13's cost-basis correction, all pre-existing and untouched by this work). No existing test was weakened or deleted to reach this number. | **Bran** | None — all 30 new checks pass against independently-verified figures, and the "8 vs 16" discrepancy is reported here rather than silently reconciled to match the brief's number. |
| 18 | **Pricing v4 Part 5 — proposal-document generator + design spec, RESOLVED by the owner 2026-08-16.** New `05-ops/render_proposal_v4.py`: standalone, offline, no-Odoo/no-DB generator producing a 10-section client HTML document and a separate INTERNAL worksheet from the same `pricing_engine.py`/`template-catalogue.yaml` functions built in Parts 1-4 — no price, hour count, or gate verdict is hand-typed in this file. **Design spec extracted from real Prosper artifacts, not invented**: colour/type from `06-brand/tokens/color.yaml` (7 client-safe tones only — `midnight`/`emerald`/`amber`/`wine` are reserved for internal status signalling per that file's own usage_notes and never used here) and `02-clients/PRO-prosper-realestate/04-draft/render_brand.py`'s typography precedent (Playfair Display + Inter, which explicitly supersedes `tokens/type.yaml`'s IBM Plex default for proposal work); A4 portrait/20mm margin grid; the exclusions-with-alternative table shape extends Prosper's real 3-column "What this does not include" table (Rev3 Offer) with a 4th Alternative-Offered column, per this brief's own requirement; signatory/legal fields sourced from `06-brand/entity/legal-identity.yaml` (single source of truth for every signature block, per that file's own header) rather than re-typed — `actual_signer` is populated (Renbran Anthony Madelo), so the `[BLOCKS ISSUE]` path exists in code but is not triggered by real data. **Two disclosed deviations from Prosper's own convention** (Part 5.1's own instruction to state these): (1) watermark rotation (`assets/watermarks/rotation.yaml`, 18 UAE-landmark assets) is NOT wired in this pass — Prosper's real precedent only skips watermarks for short single-topic letters, and this 10-section document arguably qualifies for the full treatment, but implementing 18-asset rotation logic was judged disproportionate to this pass's actual goal; (2) VAT/registration status is ALWAYS printed on the commercial-summary page here, which supersedes `AGENTS.md`'s older conditional-disclosure rule (silent unless raised) for v4 documents specifically, per this brief's direct, current instruction. **A real bug found and fixed while wiring this, not smoothed over**: the first version of `discount_gate_verdict()` (Part 2) compared every quote against the fixed 35,000-reference-quote floor regardless of what was actually being quoted — a genuinely smaller-scope fixture (platform + one module, 19,500) incorrectly triggered `COMMERCIAL_DESK_APPROVAL_REQUIRED` for being a smaller number than an unrelated reference deal, not for being discounted. Fixed by comparing each quote against 90% of its OWN scope's undiscounted total (`undiscounted_total_aed` parameter, defaulting to the reference quote only to preserve T23's exact existing call signature) — re-verified T23 still passes unchanged after the fix. Staged (NOT installed) `sgc_quotation_proposal/` — QWeb report templates only, no models, structurally mirroring the standalone generator's 10 sections and field names side by side; one real content-shape divergence found and disclosed, not papered over: the standalone generator derives each exclusion's display name from a dict key at render time (`key.replace('_',' ').title()`), while the QWeb template assumes a pre-formatted `name` field would already be present in the rendering context — the two are not byte-identical without a controller-side transform step that does not exist yet. | **Bran** | None outstanding for the generator itself. Two open items carried to Part 6/7: whether to invest in watermark rotation for v4 documents, and building the controller-side context transform (or changing the QWeb template) to close the exclusions-name divergence before this module is ever installed. |
| 19 | **Pricing v4 Part 6 — initial production test (render path, no Odoo/DB), RESOLVED by the owner 2026-08-16.** Full results in `00-intake/proposal-engine-v4-test-2026-08-16.md`. Three fixtures (F1 full-catalogue 35,000, F2 minimal 19,500, F3 migration-25,000 UNPRICED) rendered end-to-end via `render_proposal_v4.py` → `wkhtmltopdf` on `vps-root` (binary invocation only, no Odoo process, no DB) → `pypdf` page count. All three: exactly 10 pages, zero leaked internal figures, zero warrant-tier-lint hits, correct UNPRICED routing for F3. **The page-limit "10 pages" result was not accepted at face value**: since the CSS forces one page-break per section by construction, "10 pages" could be trivially true regardless of content. Built `05-ops/verify_pdf_page_limit.py` and proved it against a deliberately bloated adversarial fixture (150 injected filler lines → 22 real pages → correctly FAILed, exit 1) before trusting the real fixtures' PASS. Google Fonts resolution was checked directly (HTTP 200 from inside the render container; `PlayfairDisplay-Bold`/`Inter-Bold`/`Inter-Medium` confirmed embedded in the output PDF via `pypdf`, not a generic fallback) rather than assumed. **A real limitation disclosed, not smoothed over**: F1's internal floor-guard check computes an effective rate of 3,739 AED/hr because `template-catalogue.yaml` records an internal hour estimate for only one of the five modules (`multi_agent_access_control`, 7h) — the other four have fixed prices but no hour estimate at all, so the floor-guard "PASS" on F1 reflects one line item clearing the floor widely, not a genuine whole-deal effort check. | **Bran** | If a genuine whole-deal floor check matters before this goes further, internal hour estimates are needed for the other four modules (`lead_capture_pipeline`, `property_listing_management`, `commission_and_deals`, `reporting_and_dashboards`) — none were supplied in this pass's brief beyond the one flagged line. |
| 20 | **Origin/main merge and capability reconciliation, RESOLVED by the owner 2026-08-16.** GitHub Desktop's auto-merge surfaced an unresolved conflict: `origin/main` had 28 commits this session's local branch never had, an independent, already-tested pass at the SAME brief (business-cost-basis.yaml/`business_cost_floor()`, template-catalogue.yaml/`four_component_build()`, `commission_released()` + a real clawback clause, `capacity_table()`, `recurring_support_load_table()`, `db_guard.py`, standing self-tests T1-T19) built against a real client, `02-clients/RVN-realestate-leads/`. The four commit hashes disputed two turns earlier (`fcfeda4a`, `b6ff11d1`, `d83efd66`, `59a78f19`) were confirmed as genuine ancestors of `origin/main` specifically (`git merge-base --is-ancestor`) — the earlier "no trace anywhere" conclusion was wrong, not because the work didn't exist, but because that check never fetched the remote before searching. **Merge aborted first, not resolved live** — a read-only capability inventory was built before a single file was edited, specifically to avoid hand-merging two vocabularies for the same number into one tree, the same defect class (550 collision, R11/R12, #24) this session had already hit three times. Resolution: `git checkout --theirs` on all 5 conflicted files (origin wins wholesale — cost basis, four-component catalogue, commission/retention, capacity, recurring/support load, and the T1-T19 suite), then local-only work ported forward BY NAME onto origin's functions, not re-imported under this branch's own vocabulary: `multi_agent_access_control` added to `template-catalogue.yaml` in origin's schema (evidence: the demo_presentation estate inventory, which origin never had access to); `hour_rate_floor_test()` gained `gross_break_even_aed_hr` as a REPORTED METRIC (not a second BLOCK/WARN threshold — porting it as a threshold would have quietly lowered origin's WARN bar to zero-cushion break-even and called it a fix, since netting commission out first and comparing to the cost floor is algebraically identical to comparing gross rate against the break-even figure; origin's own 15%-cushion WARN band is a POLICY CHOICE, documented inline as such so nobody "corrects" it to a derived percentage later); `platform_portion_aed_mo()` built fresh as a real function, formalizing the recurring-subscription CTS-floor formula every client worksheet in this repo (Prosper, Kallat, VGE, MRD, RVN) has hand-computed since before either pricing-v4 pass existed — verified to reproduce Prosper's own real, already-audited figures exactly (cts_total_aed 2,918, platform_portion_aed_mo 3,648, standing regression test T20). `render_proposal_v4.py` was rebuilt, not patched, as a strict consumer of origin's functions (computes no AED figure of its own) with a genuine one-time + recurring commercial summary (horizon totals at 12/24/36 months, explicitly not compared against each other) — the prior version's four-component model was one-time-only and would have produced a commercial summary that omitted a real deal's largest line item. Fixed in the same pass, found only by actually re-rendering: the page layout was still forcing one section per page by construction (`page-break-after: always` on every section), making "10 pages" true regardless of content — real fixtures now render at 4 pages with all ten sections present (`assert_required_sections()`, a real test run on every fixture, not a styling convention), and the adversarial 22-page overflow proof was re-run and re-confirmed under the new flowing layout (27 pages, correctly failed) rather than assumed to still hold. `known-defects.md` #25 records that a local finding (5% retention base ambiguity) was independently reached and then found already-more-resolved by origin's own clause by the time of the merge; #26 records the `grep -a`-on-a-compressed-PDF trap found while fixing the RVN fixture name. | **Bran** | Deferred, not done in this pass: `sgc_quotation_proposal/`'s QWeb template still reflects the PRE-merge four-line schema and has not been updated to match the rebuilt renderer's one-time+recurring structure — flagged here so it does not silently drift further out of parity, not fixed silently. |
