...
| **SUPERSEDED — DO NOT QUOTE** — Additional users beyond 40 | **SUPERSEDED — DO NOT QUOTE** — Per-user, non-discountable | **SUPERSEDED — DO NOT QUOTE** — AED 250/user/month — `phase2-catalogue.yaml` is STALE against the 2026-08-05 v3.0 recompute, which deleted `additional_user` entirely and replaced it with `onboarding_fee_per_marginal_user` + `platform_capacity_fee` (see `phase2-catalogue.yaml`). Also pairs a per-user rate with the unsourced 40-headcount figure in one line — see manifest.yaml 2026-08-08 entry, decision #9 (reopened). Left here marked, not deleted, so the correction is visible in-place; replace with the correct Class A-D lines before this section is ever rendered or issued — Stage 5, still held. |
